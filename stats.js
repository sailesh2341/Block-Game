var Stats = function(){
    ...
    m.fillRect(0,0,a.width,a.height);y=m.getImageData(0,0,a.width,a.height);
    ...
    return{domElement:g,update:function(){
        ...
        if(j>v+1E3){
            ...
            if(t==3)p=performance.memory.usedJSHeapSize*9.54E-7,C=Math.min(C,p),D=Math.max(D,p),s(E.data,Math.min(30,30-p/2),"mb"),i.innerHTML='<span style="font-weight:bold">'+Math.round(p)+" MB</span> ("+Math.round(C)+"-"+Math.round(D)+")",q.putImageData(E,0,0);v=j;u=0
        }}
    }
};
